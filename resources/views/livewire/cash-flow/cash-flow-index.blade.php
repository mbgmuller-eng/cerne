...
<div class="space-y-6">
    ...
    <div class="flex flex-wrap items-end justify-between gap-4">
        <div>
            <h1 class="font-display text-3xl font-semibold tracking-tight text-stone-900">Fluxo de caixa</h1>
    ...
    <div class="grid gap-4 sm:grid-cols-3">
        <div class="card p-5">
            ...
            <p class="figure mt-2 text-2xl font-medium text-brand-800">{{ Money::format($totalIncome) }}</p>
        </div>

        <div class="card p-5">
            <p class="eyebrow">Despesas</p>
            <p class="figure mt-2 text-2xl font-medium text-amber-700">{{ Money::format($totalExpense) }}</p>
        </div>

        <div class="card p-5">
            <p class="eyebrow">Sobra do mês</p>
            <p class="figure mt-2 text-2xl font-medium {{ (float) $balance < 0 ? 'text-red-700' : 'text-stone-900' }}">
    ...
    @if ((float) $totalExpense > 0)
        <div class="card p-5">
            ...
            <div class="mt-3 flex h-2 overflow-hidden rounded-full bg-stone-100">
    ...
    <div class="flex flex-wrap items-end gap-3 card p-4">
        <div>
            <label class="block text-xs font-medium text-stone-500">Necessidade</label>
        ...
        <div>
            <label class="block text-xs font-medium text-stone-500">Membro</label>
        ...
        @if ($necessity || $categoryId || $memberId)
            <button wire:click="clearFilters" class="pb-1.5 text-sm text-stone-500 hover:text-brand-800">Limpar filtros</button>
        @endif
    </div>

    {{-- Despesas ------------------------------------------------------- --}}
    <section>
        <h2 class="text-sm font-semibold text-stone-900">Despesas <span class="font-normal text-stone-400">({{ $expenses->count() }})</span></h2>

        @if ($expenses->isEmpty())
            <div class="mt-3 rounded-2xl border border-dashed border-stone-300 bg-white/60 px-5 py-10 text-center">
                <p class="text-sm text-stone-500">Nenhuma despesa neste recorte.</p>
            </div>
        @else
                ...
                @foreach ($expenses as $despesa)
                    <li class="flex items-center justify-between gap-4 px-5 py-3">
                        <div class="flex min-w-0 items-center gap-3">
                            ...
                            <div class="min-w-0">
                                <p class="truncate text-sm text-stone-800">{{ $despesa->description }}</p>
                        ...
                        <div class="shrink-0 text-right">
                            <p class="text-sm tabular-nums text-stone-800">{{ Money::format($despesa->amount) }}</p>
                            @if ($despesa->isInstallment())
                                <p class="text-xs text-stone-400">parcela {{ $despesa->installmentLabel() }}</p>
                            @endif
                        </div>
                    </li>
                @endforeach
    ...
    {{-- Receitas ------------------------------------------------------- --}}
    <section>
        <h2 class="text-sm font-semibold text-stone-900">Receitas <span class="font-normal text-stone-400">({{ $incomes->count() }})</span></h2>

        @if ($incomes->isEmpty())
            <div class="mt-3 rounded-2xl border border-dashed border-stone-300 bg-white/60 px-5 py-10 text-center">
                <p class="text-sm text-stone-500">Nenhuma receita neste recorte.</p>
            </div>
        @else
            <ul class="mt-3 card divide-y divide-stone-100">
                @foreach ($incomes as $receita)
                    <li class="flex items-center justify-between gap-4 px-5 py-3">
                        <div class="min-w-0">
                            <p class="truncate text-sm text-stone-800">{{ $receita->description ?: $receita->category->name }}</p>
                            <p class="truncate text-xs text-stone-500">
                        ...
                        <p class="shrink-0 text-sm tabular-nums text-brand-800">{{ Money::format($receita->amount) }}</p>
                    </li>
                @endforeach
            </ul>
        @endif
