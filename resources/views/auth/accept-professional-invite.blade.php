<x-layouts.guest title="Criar acesso · Cerne">
    @if ($invite === null)
        ...
        <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">
            Este convite expirou ou já foi utilizado. Peça um novo ao administrador.
        </p>
        <a href="{{ route('login') }}" class="mt-6 inline-block text-sm text-brand-800 dark:text-brand-300 hover:underline">
            Ir para o login
        </a>
    @else
        <h1 class="text-lg font-semibold text-slate-900 dark:text-white">Olá, {{ $invite->name }}</h1>
        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">
            Sua conta de {{ $invite->role->label() }} no Cerne está pronta. Defina sua senha para começar.
        </p>

        <form method="POST" action="{{ route('professional-invite.store', ['token' => $token]) }}" class="mt-6 space-y-5">
            @csrf
